...
if( isset($_POST['pesan']) ){
    ...
    $durasi_langganan = intval(round((strtotime($tanggal_akhir_langganan)-strtotime($tanggal_awal_langganan))/(60*60*24)));
    if($durasi_langganan/30 <= 1){
        $durasi_langganan2 = 1;
    } else {
        $durasi_langganan2 = $durasi_langganan/30;
    }
    if($durasi_langganan <= 30){
        $harga = 250000;
    }
    elseif($durasi_langganan > 30) {
        $harga = 250000 + ($durasi_langganan2*250000);
    }
    $status = 'PENDING';

    //memasukkan ke dalam database
    $result = mysqli_query($dbcatering, "INSERT INTO berlangganan VALUES ('', '$username', '$id_toko', '$tanggal_awal_langganan', '$tanggal_akhir_langganan', '$durasi_langganan2', '$harga', '$status')");
